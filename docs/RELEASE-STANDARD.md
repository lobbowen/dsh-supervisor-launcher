...
| H4 | 构建 + 打包 | `npx --yes "@tauri-apps/cli@${TAURI_CLI_VERSION}" build --bundles "<matrix.bundles>"`（版本单点是 build.yml 该步的 `TAURI_CLI_VERSION`，由 `ci_gate_coverage_test` C-i 钉为精确三段号并禁止任何行直接引用浮动形态）| **仅 CI**（各 runner 只构建自己平台）| 是 |
...
> A 的候选按**指向的 commit** 筛，不按 ref 名：A 与本次 HEAD 是同一 commit 时一律跳过 —— 同一份内容
> 当「升级前那一份」，覆盖前后本就允许逐字节相同，那条路径等于没走；而该 Release 刚建出的那一刻资产
> 常常还没传完，拿它当基线只会撞下载失败（2026-09-26 主干 run 与 tag run 并发时即为此判红，判据由
> `installer_smoke_coverage_test.rs` 的 I-i 钉住）。
