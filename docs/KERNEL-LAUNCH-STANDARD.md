...
| K-16 | 「端口通」**不等于**「在服役」，早退/导航/面板 URL 三处必须问同一个事实（2026-09-22 真机：报完「启动完成」进面板即 127.0.0.1 拒绝连接）。形态侧（`kernel_launch_standard_test.rs`）：`ensure_guard` 的提前返回只能经 `serving_state`，按顺序链 `if port_open` → `match serving_state(port)` → 唯一的 `return Ok(())` → `Serving::SessionHalted` → `stop_and_await_release` → `ensure_started` 判定，且体内 `return Ok(())` 只许出现一次（停链分支不得早退）；`go_panel` 每一拍先复核服役、最后一拍不就绪改发 `shell:goto-bootstrap`，`shell.html` 必须接住该事件（回引导页 = 自愈入口）；`api_base_url` 优先取 `discovered_api_port()` 且不得绕道 `api_port`（后者由前者反解，会自递归）。语义侧（`guardctl.rs` 的行为用例，与 K-14 同一手法）：`fake_serving` 按路径分别应答 `/healthz` 与 `/session/status`，断言 200+active ⇒ `Alive`、503 ⇒ `Sick`、无人监听 ⇒ `Sick`、200+`stopped` ⇒ `SessionHalted`、问不出会话态 ⇒ 降级为 `Alive`（探针抖动不得升级成「停掉一个健康守卫」）。反向：旧「端口一开就早退」「无条件导航」「只认 config.json 的 apiPort」三种形态均判违规 |
