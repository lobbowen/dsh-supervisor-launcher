...
- `P5 请求服务管理器启动` 的前提是 `P4 建立成功`。定义没建立就去 `/Run` / `systemctl start`
  ...
  于是把「定义没建立」这一真实故障伪装成「启动失败」。现由 `ServiceLaunch` 承载阶段产物：
...
- P4 `ensure_defined` 失败**不阻断启动**，因为服务管理器不可用（容器 / 无 user session /
  ...
  现在 P4/P5 的结果由 `ServiceLaunch` 承载（见 §1 前置关系），定义失败 ⇒ 不发启动请求，
...
| K-8 | 产品状态根独立于 DSH（XDG），`supervisor_dir` 函数体内不得再拼 `~/.dsh`；平台默认值在 `platform/`；schema 与三个实际路径随启动写进 `shell.log` 的 `state-root schema=` 行（本壳的观测通道是日志，不是诊断 IPC） |
...
| K-15 | Windows 定义链的权限边界与通道可观测：代码里不出现 `HIGHEST`（守卫是用户态进程），守卫任务的 `/Create` 参数表只有一处装配、`create_guard_task` 无「降级重试」；只有权限类失败（`is_access_denied`）才换登录自启项通道，定义成功后写动作记录（`read_action_record`/`write_action_record`），`start` 按 `Channel::RunKey` 分派 —— Run 键不支持即时启动就如实报 Err；`start_requested` 只认 `Some(Ok(()))`（请求被拒不等于已发出请求）；兜底 `await_daemon` 必须 `child.try_wait()` 且非零退出早停 |
| K-16 | 「端口通」**不等于**「在服役」，且服役判据只许有一处实现（`guardctl::panel_view` 同时给出 URL 与「能不能投」），它必须站在**每一条**面板导航路径上（2026-09-22 真机报完「启动完成」进面板即 127.0.0.1 拒绝连接；1.2.5 只修了「判据用裸 TCP」，判据挂在 `go_panel` 上而主帧那条必然发生的导航不经它，形同虚设）。形态侧（`kernel_launch_standard_test.rs`）：`ensure_guard` 的提前返回只能经 `serving_state`，按顺序链 `if port_open` → `match serving_state(port)` → 唯一的 `return Ok(())` → `Serving::SessionHalted` → `stop_and_await_release` → `ensure_started` 判定，体内 `return Ok(())` 只许出现一次（停链分支不得早退）；`go_panel` 按 `panel_view()` → `shell:goto-panel` → `shell:goto-bootstrap` 顺序，且 `windowing.rs` 全文不得出现 `serving_state(`（判据出现第二处实现即违规），`panel_view` 函数体必须同时含 `matches!(serving_state(`、`Serving::Alive`、`api_base_url()`；`shell.html` 主帧索取侧必须在 `core.invoke('shell_panel_url')` 之后、`loadPanel(r.url)` 之前判 `r.serving === false` 并 `backToBootstrap(`，而 `backToBootstrap` 本体必须是主帧 `window.location.replace('bootstrap.html')`（写成 `iframe.src` 就是「把引导页塞进 iframe」）；稳态侧 `watch_panel` 必须含 `serving_state(` + `panel_watch_tick(` + `shell:goto-bootstrap`，`EXITING.store(true` 与决策表的 `if exiting \|\| !armed` 同时在位（退出握手不许把用户甩回引导页），`commands/mod.rs` 在 `finish_boot` 武装 `watch_panel(&app)`，更新后重载不得残留固定延时 `}, 900);` 且 `reloadPanelWhenReady` 体内必须等 `guard_ready` 并再判 `p.serving === false`；`api_base_url` 优先取 `discovered_api_port()` 且不得绕道 `api_port`（后者由前者反解，会自递归）。语义侧（`guardctl.rs` 的行为用例，与 K-14 同一手法：回环真 socket）：`fake_serving` 按路径分别应答 `/healthz` 与 `/session/status`，断言 200+active ⇒ `Alive`、503 ⇒ `Sick`、无人监听 ⇒ `Sick`、200+`stopped` ⇒ `SessionHalted`、问不出会话态 ⇒ 降级为 `Alive`（探针抖动不得升级成「停掉一个健康守卫」）；`panel_watch_bounces_only_after_consecutive_down_ticks` 钉满决策表（在位归零、连续到达门槛才回吐一次、未武装与握手中闭嘴）。反向：「端口一开就早退」「无条件导航」「主帧无条件投面板」「固定 900ms 重载」四种旧形态各有样本，任一被识别为违规才算门禁能红 |
| K-17 | 注册进 `generate_handler!` 的每条命令都必须有调用方：命令名字面量出现在 `bootstrap/` 的 html/js 里，或作为桥契约常量由 `bridge.rs` 下发给壳主帧（`kernel_update_apply` 走后者）。反向：合成文本里无人调用的命令名必须判红，已删的零调用命令（`shell_state_root`，其观测职责已并入 K-8 的启动日志行）不得回到前端。缺陷来源：该命令注册后从未被 invoke（面板是远程 http 帧，发不了 Tauri IPC），而旧 K-8 用字符串断言反向钉着它的存在 = 门禁替死代码背书 |
