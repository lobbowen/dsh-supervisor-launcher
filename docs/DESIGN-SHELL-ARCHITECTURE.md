...
> **达成度（2026-09-20 按仓库现实核对，不要把本节当已满足的结论读）**：
> - 第 1 条成立：`src-tauri/tests/` 里 G-1 ~ G-8 均有同名判据标签。
> - 第 2 条**不成立**：壳 CI 的 `build` job **从未调用** `--platform-matrix`
>   （`.github/workflows/build.yml` 无该步骤），故三平台自检目前**无人跑**。
> - 第 3 条**未达**：`src-tauri/src/main.rs` 实测 **538 行**（上限 200）。
> - 第 4 条口径已变：前端为 `src-tauri/bootstrap/js/00..80` **九个**模块，不是八个。
>
> 第 2/3 条要转成机器门禁还是修订目标值，属单独定案；在此之前它们只是**未完成的验收项**，
> 不代表壳已达标准，也不代表可以据此放行发布。
