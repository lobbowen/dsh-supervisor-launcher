...
> **本台账已过期，勿作为现状依据。** 基线提交 `0930884` 之后主干又前进了 89 个提交（v1.1.8 → v1.2.6 及
> 其后的多批次收口），本文的分级数量、`file:line` 定位与「本版已修」清单都只对当时那份树成立；其中引用的
> 154 个反引号路径当前只有 48 个仍精确存在（其余是当时的 glob 写法、内核仓路径或运行期状态文件）。
> 只保留作历史取证。现状以仓内门禁（`src-tauri/tests/*`）、`docs/DEVELOPMENT-TRACK.md` 与
> `CHANGELOG.md` 为准。
