...
## 〇、当前状态（换钥取证 2026-09-21；清单现值行每次发布重核，最近一次随 1.2.7 / 2026-09-23，先读这段）

| 事实 | 证据 |
|---|---|
| **签名产物在线上、且是在用的更新通道**：清单 `@dsh-sup/shell-release@latest` 现指 `1.2.7`（2026-09-23 实测，`pub_date` 2026-09-23T13:52:22Z；换钥后首版是 `1.2.0`），四平台各带一份 minisign 签名，**key id 逐条实测均为新钥 `54A15461E39C8AEF`**。现值随每次发布变化，重核：`npm view @dsh-sup/shell-release version` | `GET https://unpkg.com/@dsh-sup/shell-release@latest/shell-manifest.json`（`@latest` 会 302 到具体版本，须跟随；本轮落地 URL 即为 `@1.2.7/shell-manifest.json`），按 `docs/RELEASE-STANDARD.md` §5 的解法逐条取 key id（外层 base64 解出 minisign 四行，取**第二行** blob 的第 2..10 字节小端转 hex）；同期 registry `dist-tags.latest` 亦为 `1.2.7`，四平台包 `@dsh-sup/shell-{linux-x64,win-x64,darwin-arm64,darwin-x64}@1.2.7` 逐个查得。本轮清单四条 payload URL 逐个 HEAD 均 200（darwin-aarch64 3,969,671 / darwin-x86_64 4,184,519 字节，linux 与 windows 两条同取），`releases/tags/v1.2.7` = 10 项资产（4 平台签名链路产物 deb / setup.exe / msi / app.tar.gz 各带 `.sig`，另两份 mac `.dmg` 归档）。**注意时序**（1.2.4 / 1.2.5 两轮的实测，非本轮）：`publish` 完成后约 1 分钟 `releases/tags/v*` 仍可能 404、unpkg 侧滞后更久（约 3 分钟），据此断「没建 Release / 产物缺失」会是错误结论，要隔一分钟复取 |
