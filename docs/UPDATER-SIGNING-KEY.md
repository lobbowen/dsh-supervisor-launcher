...
| 事实 | 证据 |
|---|---|
| **签名产物在线上、且是在用的更新通道**：清单 `@dsh-sup/shell-release@latest` 现指 `1.2.3`（2026-09-22 实测，`pub_date` 2026-09-22T10:05Z；换钥后首版是 `1.2.0`），四平台各带一份 minisign 签名，**key id 逐条实测均为新钥 `54A15461E39C8AEF`**。现值随每次发布变化，重核：`npm view @dsh-sup/shell-release version` | `GET https://unpkg.com/@dsh-sup/shell-release@latest/shell-manifest.json`（`@latest` 会 302 到具体版本，须跟随），按 `docs/RELEASE-STANDARD.md` §5 的解法逐条取 key id；本轮同查 `releases/tags/v1.2.3` = 10 项资产（4 平台安装包 + 4 份 `.sig` + 两份 mac app 归档）|
