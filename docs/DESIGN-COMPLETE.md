...
> **达成度与口径冲突（2026-09-20 按仓库现实核对）**：逐条核对结果记在
> `DESIGN-SHELL-ARCHITECTURE.md` §八 的现状块里（`--platform-matrix` 无 CI 步骤、
> `main.rs` 实测 538 行、前端为九个模块而非八个）。此处另有一处**两份「权威」文档互不相同**：
> 本条第 3 项写 `main.rs ≤ 150 行`，架构规范写 `≤ 200 行`，且**没有任何门禁执行这两个数**。
> 保留两个不同的目标值本身就是错误引导；定案前把本节当**未完成清单**读。
