//! 安装与下载进度的**唯一**语义层（SSOT §2.4 事件形态 / §3.2 文字出口）。
//!
//! 为什么单独成模块（2026-09-21 B4）：`install_progress` 此前有**两个**发射点 ——
//!   `main.rs::push_status` 与桌面壳更新在 `commands/mod.rs` 里内联的 `json!`；
//!   `kind` 又只枚举了 node/npm，内核与桌面壳用裸字符串过线，于是
//!   「哪些东西可被安装」在 Rust 与前端各有一份账（前端 `10-ui.js` 的 `INSTALL_TARGET` 有四个）。
//!   更要紧的是**没有真进度源**：内核安装（`npm install`，上限 15 分钟）一个字都不发，
//!   Node 归档下载只发一句写死的「约 30~50MB…」，而阶段分数（0.1 / 0.3 / 0.85）
//!   是按代码顺序编出来的 —— 与真实进度无关。前端删掉进度条正是因为这种数字会骗人，
//!   那它们就不该继续出现在线上协议里。
//!
//! 本模块因此只承认两种进度：**可测分母**（下载字节比）与**如实的阶段/心跳文字**。
//! 无分母时 `progress` 发 `null`，不猜。

use tauri::{Emitter, Manager};

/// 可被安装/下载的东西 —— 统一事件 `kind` 字面量的**唯一**来源（SSOT §2.4 冻结四类）。
///
/// 为什么用枚举而不是裸字符串：`start_node_install` 必须把失败**如实归给**出问题的步骤
///   （node 装不上 / npm 补不上），否则前端会拿错文案前缀，把「缺 npm」显示成「装 Node 失败」。
///   同理，内核与桌面壳也不再各写一份 `"kernel"` / `"shell"` 字面量。
#[derive(Clone, Copy)]
...
/// 下载行的**唯一**渲染点（纯函数，可离线穷举）。
///
/// `total` 为 `None` 或**小于已取回量**时只报已取回量、比值发 `null`：
///   `into_reader()` 在 `Transfer-Encoding: chunked` 时会忽略 Content-Length 一路读到流结束
///   （ureq 文档），于是服务端声称的总量可能偏小。文案自相矛盾比少一根进度条糟糕得多。
...
/// npm 安装期心跳文案（**唯一**渲染点）。
///
/// 为什么长这样：`npm install` 不吐百分比，输出又被重定向到临时文件（不经管道，见
...
/// 内核安装的**开工行**：把「在装什么、有几个源、上限多久」一次说清。
///
/// 为什么这些数字必须出现在文案里：内核安装的耗时上界是**已知的**（单源 `core::NPM_INSTALL_TIMEOUT`
///   × 逐源 + 总预算 `bridge::KERNEL_UPDATE_BUDGET_MS`），而此前引导页只有一句「正在强制更新…」——
///   用户报的「看不到下载进度 / 不知道是不是卡住」，缺的从来不是进度条，是**说清预算**。
...
pub(crate) fn run_install(app: &tauri::AppHandle) -> Result<crate::runtime_contract::NodeRuntime, InstallFailure> {
    ...
    // ① 解析并安装/修复 node（latest_lts → download_verified → install 链路不变）。
